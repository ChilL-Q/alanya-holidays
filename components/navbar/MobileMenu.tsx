...
export const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="absolute top-20 right-0 left-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shadow-2xl p-4 flex flex-col gap-4 z-40 animate-in slide-in-from-top-10 duration-300 md:hidden max-h-[85vh] overflow-y-auto">
            ...
            <div className="flex flex-col gap-1">
                {/* Main Nav */}
                <Link to="/stays" onClick={onClose} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-medium text-slate-700 dark:text-slate-200">
                    <Home size={18} className="text-slate-400" />
                    {t('nav.stays')}
                </Link>
                <Link to="/services" onClick={onClose} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-medium text-slate-700 dark:text-slate-200">
                    <Car size={18} className="text-slate-400" />
                    {t('nav.services')}
                </Link>
                <Link to="/shop" onClick={onClose} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-medium text-slate-700 dark:text-slate-200">
                    <ShoppingBag size={18} className="text-slate-400" />
                    {t('shop')}
                </Link>
                <Link to="/zero-fees" onClick={onClose} className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-bold text-teal-600 dark:text-teal-400 bg-teal-50/50 dark:bg-teal-900/10">
                    <Banknote size={18} />
            ...
            <div className="flex gap-2">
                <Link to="/list-property" onClick={onClose} className="flex-1 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl flex flex-col items-center justify-center gap-2 text-center group active:scale-95 transition-transform">
                        ...
                        <div className="grid grid-cols-3 gap-2 px-2 animate-fade-in">
                            {(['USD', 'EUR', 'TRY'] as Currency[]).map((curr) => (
                                <button
                                    ...
                                    className={`py-2 rounded-lg text-sm font-medium border ${currency === curr ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300' : 'border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}
                                >
                                    {curr}
                                </button>
                            ))}
                        ...
                        <div className="grid grid-cols-2 gap-2 px-2 animate-fade-in">
                            {[
                                ...
                                <button
                                    ...
                                    className={`py-2 px-3 text-left rounded-lg text-sm font-medium border ${language === l.code ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-200 dark:border-teal-800 text-teal-700 dark:text-teal-300' : 'border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-400'}`}
                                >
                                    {l.label}
                                </button>
                            ))}
