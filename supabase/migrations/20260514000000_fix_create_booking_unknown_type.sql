-- Fix create_booking: reject unknown item_type instead of silently falling back to 'service'.
-- The previous migration allowed unknown types to pass through and be stored as 'service'.
-- This migration tightens validation to prevent incorrect data classification.
...
BEGIN
    ...
    ELSE
        RETURN jsonb_build_object('error', 'Invalid item type. Must be property, service, or product');
    END IF;
