-- Purpose: Add auth.uid() check to get_directory_analytics_for_owner
-- Without this, any authenticated user could pass an arbitrary p_owner_id
-- and view another owner's per-listing analytics.
...
BEGIN
    IF p_owner_id != auth.uid() THEN
        RAISE EXCEPTION 'Unauthorized: can only view own analytics';
    END IF;
