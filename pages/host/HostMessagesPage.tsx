...
export const HostMessagesPage: React.FC = () => {
    ...
    return (
        <div className="flex h-[calc(100vh-210px)] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xl mt-6 mx-4 md:mx-0">
            {/* Sidebar List */}
            <div className={`${activeConversationId ? 'hidden md:flex' : 'flex'} w-full md:w-96 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900`}>
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-slate-900 dark:text-white font-serif">Messages</h2>
                        <span className="bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-400 text-xs font-bold px-2.5 py-1 rounded-full">
                    ...
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all shadow-sm"
                        />
                    </div>
                </div>
                                ...
                                return (
                                    ...
                                    >
                                            ...
                                            <div className="flex items-center gap-3">
                                                <div className="relative">
                                                    {conv.guest?.avatar_url ? (
                                                        ...
                                                        <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center ring-2 ring-white dark:ring-slate-900">
                                                ...
                                                <div>
                                                    <h3 className={`font-bold text-sm leading-tight ${isActive ? 'text-teal-900 dark:text-teal-50' : 'text-slate-900 dark:text-white'}`}>
                                        ...
                                        <div className="flex justify-between items-center pl-[52px] relative z-10">
                                            <p className={`text-xs truncate max-w-[200px] leading-relaxed ${isActive ? 'text-teal-700/80 dark:text-teal-300/70 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                                        ...
                                        {isActive && (
                                            <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-1 bg-teal-500 rounded-r-full"></div>
                                        )}
                                    </button>
                                );
                            })}
            ...
            <div className={`${activeConversationId ? 'flex' : 'hidden md:flex'} flex-1 flex-col bg-slate-50/50 dark:bg-slate-900/80 relative`}>
                ...
                {activeConversationId ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-slate-600 animate-in fade-in duration-500">
                        <div className="w-24 h-24 bg-teal-50 dark:bg-slate-800 rounded-[2rem] flex items-center justify-center mb-6 shadow-sm rotate-3 transform transition-transform hover:rotate-6">
                            <MessageSquare size={48} className="text-teal-500/50" />
                        </div>
