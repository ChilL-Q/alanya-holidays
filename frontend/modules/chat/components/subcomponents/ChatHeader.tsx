...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
    ...
    return (
        <div className={`relative z-10 p-4 ${embedded ? 'bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800/50' : 'bg-gradient-to-r from-teal-600 to-emerald-600 text-white shadow-lg'}`}>
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        {otherPerson?.avatar_url ? (
                            ...
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-inner ${embedded ? 'bg-teal-50 text-teal-600 dark:text-cyan-400 ' : 'bg-white/10 text-white backdrop-blur-sm'}`}>
                                <User size={20} />
                            </div>
                        )}
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`p-2 rounded-full transition-colors relative ${embedded
                                ? 'text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800/90'
                        ...
                        {showMenu && (
                            <div className="absolute top-10 right-0 w-56 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800/50 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-200">
                                <button
                                    onClick={async () => {
                                    ...
                                    className="w-full text-left px-4 py-2.5 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800/90 transition-colors flex items-center gap-2 group/btn"
                                >
                                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800/80 flex items-center justify-center group-hover/btn:bg-slate-200 dark:group-hover/btn:bg-slate-700 transition-colors">
