...
describe('BlogPostPage', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (blogService.getBlogPost as any).mockResolvedValue(mockPost);
    (blogService.getRelatedPosts as any).mockResolvedValue([]);
  });

  it('renders loading state initially', async () => {
    (blogService.getBlogPost as any).mockImplementation(
  ...
  it('fetches blog post on mount', async () => {
    ...
    await waitFor(() => {
      expect(blogService.getBlogPost).toHaveBeenCalledWith('test-slug', true);
    });
  });
  ...
  it('handles missing blog post gracefully', async () => {
    (blogService.getBlogPost as any).mockResolvedValue(null);
  ...
  it('does not render TOC if there are less than 3 headings', async () => {
    ...
    (blogService.getBlogPost as any).mockResolvedValue(postWithFewHeadings);
  ...
  it('renders TOC and assigns slugified IDs to headings when there are 3 or more headings', async () => {
    ...
    (blogService.getBlogPost as any).mockResolvedValue(postWithHeadings);
  ...
  it('calls getRelatedPosts with correct arguments when post is loaded', async () => {
    ...
    await waitFor(() => {
      expect(blogService.getRelatedPosts).toHaveBeenCalledWith('post-1', 'Travel', 3);
    });
  });
  ...
  it('renders related posts section when related posts are returned', async () => {
    ...
    (blogService.getRelatedPosts as any).mockResolvedValue(mockRelatedPosts);
