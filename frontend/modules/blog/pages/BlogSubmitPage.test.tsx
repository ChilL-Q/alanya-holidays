...
import { storageService, blogService } from '../../../api-services';
...
describe('BlogSubmitPage', () => {
  ...
  it('calls createBlogSubmission on valid submission', async () => {
    (storageService.uploadBlogMediaBatch as any).mockResolvedValue(['https://example.com/image1.jpg']);
    (blogService.createBlogSubmission as any).mockResolvedValue(undefined);
    ...
    await waitFor(() => {
      expect(blogService.createBlogSubmission).toHaveBeenCalledWith({
        title: 'A Valid Title',
  ...
  it('navigates to success page on successful submission', async () => {
    (storageService.uploadBlogMediaBatch as any).mockResolvedValue([]);
    (blogService.createBlogSubmission as any).mockResolvedValue(undefined);
  ...
  it('shows loading state while submitting', async () => {
    (storageService.uploadBlogMediaBatch as any).mockResolvedValue([]);
    (blogService.createBlogSubmission as any).mockImplementation(() => new Promise(resolve => setTimeout(() => resolve(undefined), 100)));
  ...
  it('shows error toast on submission failure', async () => {
    (storageService.uploadBlogMediaBatch as any).mockResolvedValue([]);
    (blogService.createBlogSubmission as any).mockRejectedValue(new Error('Database error'));
