...
export const ForumPostPage: React.FC = () => {
    ...
    const load = useCallback(async () => {
        ...
        setLoading(true);
        try {
            const p = await forumService.getForumPost(slug);
            if (!p) {
                setNotFound(true);
                return;
            }
            setPost(p);
            forumService.incrementPostView(p.id);
            const c = await forumService.getForumComments(p.id);
            setComments(c);
        } catch (e) {
    ...
    const handleSubmitComment = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const created = await forumService.createForumComment(post.id, newComment.trim());
    ...
    const handleReport = async (targetType: ForumReportTargetType, targetId: string) => {
        ...
        if (!reason) return;
        try {
            await forumService.reportContent({ target_type: targetType, target_id: targetId, reason });
