        ...
        return (
            <div className="mt-12 pt-10 border-t border-slate-100 dark:border-slate-800/50">
                ...
                <div className="text-center py-12 bg-slate-50 dark:bg-slate-800/30 rounded-lg border border-dashed border-slate-200 dark:border-slate-700/50">
                    ...
                    <p className="text-slate-500 dark:text-slate-400">No related listings yet</p>
                </div>
            </div>
        );
    }
        ...
        <div className="mt-12 pt-10 border-t border-slate-100 dark:border-slate-800/50">
            ...
            <div className="space-y-3">
                {listings.map((listing) => (
                    <Link
                        key={listing.id}
                        to={`/directory/${listing.category_id}/${listing.id}`}
                    ...
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div className="flex-1 min-w-0">
                                ...
                                {listing.location && (
                                    <p className="text-xs text-slate-500 dark:text-slate-500 mt-1 inline-flex items-center gap-1">
                                        <MapPin size={12} />
                                        {listing.location}
                                    </p>
                                )}
                                ...
                                <div className="inline-flex items-center gap-0.5">
                                    {Array.from({ length: 5 }).map((_, i) => (
                                        <Star
                                            key={i}
                                            size={12}
                                            className={i < Math.round(listing.reviews_average || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-300 dark:text-slate-600'}
                                        />
                                    ))}
                                </div>
                                {listing.reviews_count ? `${listing.reviews_count} reviews` : 'No reviews'}
                            </div>
                        )}
                    </Link>
                ))}
