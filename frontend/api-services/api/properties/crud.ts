...
export async function getPropertiesByIds(ids: string[]) {
    ...
    return fetchJson<PropertyDB[]>('/api/properties/by-ids', {
        method: 'POST',
...
export async function createProperty(data: Omit<PropertyDB, 'id' | 'created_at' | 'updated_at'>) {
    ...
    return fetchJson<PropertyDB>('/api/properties', {
        method: 'POST',
...
export async function getProperties(
    ...
    return fetchJson<{ data: PropertyDB[], count: number }>(`/api/properties?${params.toString()}`);
}

export async function getAvailableProperties(checkIn: string, checkOut: string) {
    return fetchJson<PropertyDB[]>('/api/properties/available', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export async function getProperty(id: string) {
    return fetchJson<PropertyDB>(`/api/properties/${id}`);
}

export async function getPropertiesByHost(hostId: string) {
    return fetchJson<PropertyDB[]>(`/api/properties/host/${hostId}`);
}
...
export async function getAdminProperties(statusFilter = 'all', page = 1, limit = 50) {
    ...
    return fetchJson<{ data: PropertyDB[], count: number }>(`/api/properties/admin?${params.toString()}`, { headers });
}
