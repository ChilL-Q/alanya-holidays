...
export class Money {
  ...
  static fromCents(cents: number, currency: string = 'EUR'): Money {
    ...
    // Replace internal cents with exact integer cents
  ...
  static parse(value: string | number, fallbackCurrency = 'EUR'): Money {
    ...
    else if (value.includes('$') || /USD/i.test(value))
      detectedCurrency = 'USD';
    else if (value.includes('₺') || /TRY|TL/i.test(value))
  ...
  subtract(other: Money): Money {
    ...
    if (this._cents < other.cents) {
      throw new Error('Invalid money operation: result cannot be negative');
    }
    return Money.fromCents(this._cents - other.cents, this._currency);
  }
  ...
  /**
   * Returns decimal representation for database persistence.
   */
