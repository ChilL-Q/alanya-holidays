...
describe('DirectoryController', () => {
  ...
  beforeEach(async () => {
    mockService = {
      ...
      createAddonCheckout: jest.fn().mockResolvedValue({ url: 'http://checkout' }),
      sendListingPaymentInstructions: jest.fn().mockResolvedValue({ success: true }),
  ...
  it('should pass query parameters to searchDirectoryListings', async () => {
    await controller.searchDirectoryListings('beach', 'cat-1', 'Alanya', '2', '20');
  ...
  it('should pass req.user.id to approveDirectoryListing', async () => {
    ...
    expect(mockService.approveDirectoryListing).toHaveBeenCalledWith('dir-9', 'admin-77');
  });

  it('should pass req.user.id to voteForListing', async () => {
    const req = { user: { id: 'usr-88' } };
    await controller.voteForListing('dir-9', 1, req);
    expect(mockService.voteForListing).toHaveBeenCalledWith('dir-9', 1, 'usr-88');
  });
});
